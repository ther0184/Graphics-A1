...
namespace GameDev2D
{
	...
	void Drawable::CalculateEdges()
	{
		...
		m_Edges[LeftEdge] = m_Position.x + sh * (ay - 0.5) - cw * (ax - 0.5) - ex;
		m_Edges[RightEdge] = m_Position.x + sh * (ay - 0.5) - cw * (ax - 0.5) + ex;
		m_Edges[TopEdge] = m_Position.y - sw * (ax - 0.5) - ch * (ay - 0.5) + ey;
		m_Edges[BottomEdge] = m_Position.y - sw * (ax - 0.5) - ch * (ay - 0.5) - ey;
